...
function renderMrrPrint(docNo, info, items) {
  ...
  // Build items HTML — 7 columns ONLY (NO QR column in MRR table)
  var itemsHtml = '';
  if (items && items.length > 0) {
    items.forEach(function(it, idx) {
      var code = it.itemCode || it.inventoryId || it.code || it.id || '';
      var desc = it.description || it.desc || it.itemDescription || '—';
      var recQty = it.recQty != null ? it.recQty : (it.expectedQty != null ? it.expectedQty : (it.qty || 0));
      var atlQty = it.atlQty != null ? it.atlQty : (it.actualQty != null ? it.actualQty : (it.issuedQty || 0));
  ...
  var html = '<div class="mrr-print-sheet">' +
    ...
    '<div class="mrr-sigs">' +
      ...
      '<div class="mrr-sig"><div class="mrr-sig-name"></div><div class="mrr-sig-line"></div><div class="mrr-sig-label">NOTED BY</div></div>' +
    '</div>' +
  '</div>';
